...
<template>
  <PageLinks>
    <template #links>
      ...
      <router-link :to="`/horoscope/druids/sign/${foundSign.en}`">{{ foundSign.ru }}</router-link>
    </template>
  </PageLinks>
  <DefaultDescSign :sign="foundSign"></DefaultDescSign>
  <CheckDate :horoscopeType="'druids'" :title="'Японский'"  :mb="40"></CheckDate>
  <CheckSign :signsData="signs" :title="'Описание знаков в Японском гороскопе'" :horoscopeType="'druids'"></CheckSign>
</template>
